...
- **Verifiable session** (`src/anchor.mjs`): `z0g share` bundles the transcript + provenance and uploads it to 0G Storage (returning a content root); `z0g share --anchor` writes that root to 0G Chain.
